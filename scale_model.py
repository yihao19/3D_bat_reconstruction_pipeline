...
class Model(nn.Module):
    def __init__(self, template_obj_path,bone_skining_matrix_path='./new_bat_params_version2_foreward_membrane.pkl', train_skining_matrix = False, opposite_direction=True):
    ...
    def forward(self, batch_size, estimated_location, use_previous = False, prev_pose = None):
        ...
        if(use_previous == True):
            ...
            self.pose_tensor[1][:] = ( prev_pose[1][:] + math.pi / 9 * torch.tanh(self.joint_1))
           ...
           # self.pose_tensor[29][:] = ( prev_pose[29][:] + math.pi / 18 * torch.tanh(self.joint_29))
            ...
            if(self.opposite_direction == True):
                ...
                self.pose_tensor[0][1] = math.pi / 2 * torch.tanh(self.joint_0[0][1]) + math.pi # flip the template
            ...
            self.pose_tensor[18][0] = math.pi / 3 * torch.tanh(self.joint_18[0][0])
            ...
            self.pose_tensor[19] =  math.pi / 3 * torch.tanh(self.joint_19)
            
            
            
            #self.pose_tensor[7][:] = math.pi / 3 * torch.tanh(self.joint_7)
            #self.pose_tensor[20][:] = math.pi / 3 * torch.tanh(self.joint_20)
            ...
            self.pose_tensor[30][:] = math.pi /  6 * torch.tanh(self.joint_30)
            #self.pose_tensor[31][:] = math.pi / 18 * torch.tanh(self.joint_31)
            
            self.pose_tensor[32][:] = math.pi /  6 * torch.tanh(self.joint_32)
        ...
        joints_tail = 0.0035 * self.scale * joints_tail + estimated_location[0].repeat(1, self.joint_number, 1).cuda() + 0.1 * torch.tanh(self.displacement.repeat(1, self.joint_number, 1)).cuda() 
        ...
        bone_prior_1 = 0.05 * (torch.norm(self.pose_tensor[4]) + torch.norm(self.pose_tensor[17])+torch.norm(self.pose_tensor[5])+torch.norm(self.pose_tensor[18]))
        
                                                                                                           
        bone_prior_2 = 0.1 * (torch.norm(self.pose_tensor[6]) + torch.norm(self.pose_tensor[19]) + torch.norm(self.pose_tensor[30]) + torch.norm(self.pose_tensor[32]))
                     
        bone_prior_3 = 1 * (torch.norm(self.pose_tensor[8]) 
        ...
        bone_symmetric = 0.5 * bone_symmetric_1 + 0.3 * bone_symmetric_2 + 10 * bone_symmetric_3
...
def main(test_name, passed_pose_index, epoch, use_previous, opposite_direction):
    ...
    output_path = 'D:/PhDProject_real_data/{}/rearrange_pose/{}/'.format(test_name, pose_index)
    #camera_list = [1]
    camera_meta_path = 'D:/PhDProject_real_data/{}/rearrange_pose/'.format(test_name)
    camera_list_path = 'D:/PhDProject_real_data/{}/rearrange_pose/{}/'.format(test_name, pose_index)
    silouettee_image_path = 'D:/PhDProject_real_data/{}/rearrange_pose/{}/'.format(test_name, pose_index)
    #estimated_location_file = 'G:/PhDProject_real_data/{}/rearrange_pose/{}/estimated_location.txt'.format(test_name, pose_index)
    args.output_dir  = 'D:/PhDProject_real_data/{}/reconstruction/'.format(test_name)
    ...
    for i in epoch:
        ...
        for training_sample in train_dataloader:
            ...
            loss = IOU_loss + pose_loss  + 0 * laplacian_loss + 10 * wing_tip_reg  + 0.1 * bone_prior + 0.05 * bone_symmetric
            epoch.set_description('IOU Loss: %.4f   Pose Loss: %.4f  Wingtip_reg: %.4f  Bone prior: %.4f  Bone symmetry: %.4f' % (IOU_loss.item(),pose_loss.item(), wing_tip_reg.item(), 0.1 * bone_prior.item(), bone_symmetric.item()))
...
if __name__ == '__main__':
    ...
    for pose_index in range(start_pose, end_pose + interval, interval):
        ...
        main(test_name, pose_index, epoch, use_previous = True, opposite_direction = True)
