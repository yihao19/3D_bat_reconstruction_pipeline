...
if __name__=="__main__":
     ...
     for counter in range(start_index, end_index+1): 
         ...
         if(math.isnan(validation_location[0])): 
             continue
         print(counter, " ",    np.linalg.norm((joint_location - validation_location)))
         total_distance +=     np.linalg.norm((joint_location - validation_location))
         valid_frame_counter += 1
         #print(distance)
         index.append(counter-start_index)
         loss.append(np.linalg.norm((joint_location - validation_location) * 100.0 * 4.8))
     ...
     for counter in range(start_index, end_index+1): 
         ...
         post_process.append(100* 4.8 * np.linalg.norm(joint_location - validation_location))
         total_distance += 100* 4.8 * np.linalg.norm(joint_location - validation_location)
         valid_frame_counter += 1
         #print(distance)
     
     print(len(post_process), len(loss))
     #index = [counter for counter in range(81)]
     f = plt.figure()
     ax = f.add_subplot()
     ax.plot(loss[:], color='black')
     ax.plot(post_process[:], '--')
     plt.savefig("3D_validation_single_right_wrist.svg")
